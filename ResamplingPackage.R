...
RSSampleLoop <- function(Rating,
    ...
    for (trait in traits) {
        ...
        sampled.trait <- cbind(Trait = rep(trait, nrow(sampled.trait)), sampled.trait)
        ci.trait      <- cbind(Trait = rep(trait, nrow(ci.trait)), ci.trait)
    ...
    cat(paste("Total time: ", round(time.total["elapsed"] / 60 / 60, 2), " hours.", sep =
                  ""))
    cat("\n")
